...
# create External network
    ...
    # create server
        ...
        self.router_interface = self.client.list_ports(device_owner="network:router_interface",
                                                network_id=self.network['id'])[1]['ports'][0]['fixed_ips'][0]['ip_address']
        if self._external:
...
# Test Record PTR EA's for Router Gateway
    ...
    @test.attr(type='smoke')
    def test_EA_External_router_gateway_PTR_CMP_Type(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_External_router_gateway_PTR_name(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
        external_port = "router-gw-" + \
            self.router_gateway.replace(".", "-") + "." + fqdn
        args = "ptrdname=%s" % (external_port)
        code, msg = self.ib.wapi_get_request("record:ptr", args)
        if code == 200 and len(loads(msg)) > 0:
            self.assertEqual(loads(msg)[0]['ptrdname'], external_port)
        else:
            self.fail("NS_group %s is not used in NIOS" % external_port)

    @test.attr(type='smoke')
    def test_EA_External_router_gateway_PTR_Port_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_External_router_gateway_PTR_IP_Type(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_External_router_gateway_PTR_device_id(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
...
# Test 'PTR Record' External network DHCP port EA's

    @test.attr(type='smoke')
    def test_EA_External_dhcp_PTR_Port_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_External_dhcp_PTR_Device_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_External_dhcp_PTR_CMP_Type(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_External_dhcp_PTR_Tenant_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
...
#Test Floating IP 'A Record' EA's
...
#Test Floating IP 'PTR Record' EA's

    @test.attr(type='smoke')
    def test_EA_Floating_IP_PTR_VM_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_Floating_PTR_IP_Type(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_Floating_IP_PTR_VM_Name(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_Floating_IP_PTR_Port_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_Floating_IP_PTR_Device_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_Floating_IP_PTR_Account(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
    ...
    @test.attr(type='smoke')
    def test_EA_Floating_IP_PTR_Tenant_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(
            self.Ext_network,
            self.ext_subnet,
            external=True)
...
#Test Private Network DHCP Port 'A Record' EA's
...
# Test Private Network PTR DHCP Port EA's

    @test.attr(type='smoke')
    def test_EA_Private_Network_dhcp_PTR_Tenant_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
        dhcp_port = "dhcp-port-" + \
            self.internal_dhcp_ip.replace(".", "-") + "." + fqdn
    ...
    @test.attr(type='smoke')
    def test_EA_Private_Network_dhcp_PTR_Port_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
        dhcp_port = "dhcp-port-" + \
            self.internal_dhcp_ip.replace(".", "-") + "." + fqdn
        args = "ptrdname=%s&_return_fields=extattrs" % (dhcp_port)
    ...
    @test.attr(type='smoke')
    def test_EA_Private_Network_dhcp_PTR_IP_Type(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
        dhcp_port = "dhcp-port-" + \
            self.internal_dhcp_ip.replace(".", "-") + "." + fqdn
        args = "ptrdname=%s&_return_fields=extattrs" % (dhcp_port)
...
# Test EA's for Router Gateway
    ...
    @test.attr(type='smoke')
    def test_EA_internal_router_interface_CMP_Type(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
    ...
    @test.attr(type='smoke')
    def test_EA_internal_router_interface_Port_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
        external_port = "router-iface-" + \
            self.router_interface.replace(".", "-") + "." + fqdn
        args = "name=%s&_return_fields=extattrs" % (external_port)
        ...
        if code == 200 and len(loads(msg)) > 0:
            self.assertEqual(loads(msg)[0]['extattrs']['Port ID']['value'],
                self.admin_client.list_ports(device_owner="network:router_interface")[1]['ports'][0]['id'])
        else:
            self.fail("Port ID for %s is not added in NIOS" % external_port)

    @test.attr(type='smoke')
    def test_EA_internal_router_interface_IP_Type(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
        external_port = "router-iface-" + \
            self.router_interface.replace(".", "-") + "." + fqdn
        args = "name=%s&_return_fields=extattrs" % (external_port)
        code, msg = self.ib.wapi_get_request("record:a", args)
        if code == 200 and len(loads(msg)) > 0:
            self.assertEqual(loads(msg)[0]['extattrs']['IP Type']['value'], "Fixed")
        else:
            self.fail("IP Type EA is mismatch for %s port in NIOS" % external_port)

    @test.attr(type='smoke')
    def test_EA_internal_router_interface_device_id(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
        external_port = "router-iface-" + \
            self.router_interface.replace(".", "-") + "." + fqdn
        args = "name=%s&_return_fields=extattrs" % (external_port)
        ...
        if code == 200 and len(loads(msg)) > 0:
            self.assertEqual(loads(msg)[0]['extattrs']['Port Attached Device - Device ID']['value'],
                self.admin_client.list_ports(subnet_id=self.ext_subnet['id'], device_owner="network:router_interface")[1]['ports'][0]['device_id'])
        else:
            self.fail("EA for Port Attached Device - Device ID % does not match with NIOS" %
...
# Test EA's for Router Gateway
    ...
    @test.attr(type='smoke')
    def test_EA_internal_router_interface_PTR_CMP_Type(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
        external_port = "router-iface-" + \
            self.router_interface.replace(".", "-") + "." + fqdn
        args = "ptrdname=%s&_return_fields=extattrs" % (external_port)
    ...
    @test.attr(type='smoke')
    def test_EA_internal_router_interface_PTR_Port_ID(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
        external_port = "router-iface-" + \
            self.router_interface.replace(".", "-") + "." + fqdn
        args = "ptrdname=%s&_return_fields=extattrs" % (external_port)
    ...
    @test.attr(type='smoke')
    def test_EA_internal_router_interface_PTR_IP_Type(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
        external_port = "router-iface-" + \
            self.router_interface.replace(".", "-") + "." + fqdn
        args = "ptrdname=%s&_return_fields=extattrs" % (external_port)
    ...
    @test.attr(type='smoke')
    def test_EA_internal_router_interface_PTR_device_id(self):
        fqdn = self.ib.get_fqdn_from_domain_suffix_pattern(self.network, self.subnet)
        external_port = "router-iface-" + \
            self.router_interface.replace(".", "-") + "." + fqdn
        args = "ptrdname=%s&_return_fields=extattrs" % (external_port)
