...
sed -i "s/OS_PASSWORD=.*/export OS_PASSWORD=infoblox/" ~/keystonerc_admin
source ~/keystonerc_admin
...
if [ ! -d $Tempest_Master ]; then
    git clone http://10.39.23.201:8080/tempest-infoblox
fi
...
source ~/keystonerc_admin
IP=`ifconfig br-ex | sed -n 2p |cut -d ':' -f2|awk '{print $1}'`
